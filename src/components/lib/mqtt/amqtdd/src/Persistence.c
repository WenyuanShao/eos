...
property brokerProps[] =
{
	...
	{ "autosave_interval", 0, offsetof(BrokerStates, autosave_interval) },
...
property bridgeProps[] =
{
	...
	{ "reconnect_interval", 0, offsetof(BridgeConnections, start_reconnect_interval) },
	{ "max_reconnect_interval", 0, offsetof(BridgeConnections, max_reconnect_interval) },
	{ "connect_timeout", 0, offsetof(BridgeConnections, connect_timeout) },
};
/**
					...
					else
					{
...
#if defined(MQTTMP)
								//	else if (strcmp(val, "mqtt-multiplex") == 0)
								//		((Listener*)s)->protocol = PROTOCOL_MQTT_MP;
#endif
#if defined(MQTTS)
						...
						if (strcmp(pword, "protocol") == 0)
						{
							...
							else if (strcmp(val,"mqtts") == 0)
							{
								...
								((BridgeConnections*)s)->try_private = 0;
							}
						}
						else
#endif
...
{
	return Persistence_open_common(mode, "amqtdd.rms", "amqtdd.1ms", "amqtdd.2ms");
}
...
{
	return Persistence_open_common(mode, "amqtdd.sub", "amqtdd.1ub", "amqtdd.2ub");
}
		...
		if (fread(&(len), sizeof(int), 1, rfile) == 1)
		{
				...
				fread(&len, sizeof(int), 1, rfile) == 1)
			{
				...
				if (fread(s->topicName, len, 1, rfile) == 1)
				{
					...
					if ((elem = ListFindItem(bstate->clients, s->clientName, clientIDCompare)) == NULL)
						ListAppend(bstate->clients, Persistence_createDefaultClient(s->clientName),
