...
void MQTTSProtocol_timeslice(int sock)
{
	...
	//printc("\tMQTTSPacket factory done\n");
	if (clientAddr)
		client = Protocol_getclientbyaddr(clientAddr);
	//printc("\tclientAddr: %s, %x\n", clientAddr, client);

#if !defined(NO_BRIDGE)
	if (client == NULL)
		client = Protocol_getoutboundclient(sock);
	//printc("\toutbound client process done\n");
#endif
	...
	{
		printc("\t??????header_type: %d\n", (int)pack->header.type);
	...
	//printc("MQTTSProtocol_timeslice done\n");
	FUNC_EXIT;
}
