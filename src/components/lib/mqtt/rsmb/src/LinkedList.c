...
ListElement* ListFindItem(List* aList, void* content, int(*callback)(void*, void*))
{
	...
	if (aList->count == 0)
		;
	else if (aList->current != NULL && ((callback == NULL && aList->current->content == content) ||
...
/**
 * Removes and frees the first item in a list, appending an item to the end.
 * @param aList the list from which the item is to be removed
 */
void ListRemoveHeadAddTail(List* aList, void* content, int size)
{
	if (aList->count == 0)
		ListAppend(aList, content, size);
	else
	{
		ListElement* first = aList->first;
		free(first->content);
		first->content = content;
		/* todo: adjust list size total */
		first->size = size;
		if (aList->count > 1)
		{
			if (aList->current == first)
				aList->current = first->next;
			/* move first to last */
			aList->first->prev = aList->last;
			aList->last->next = aList->first;
			aList->last = aList->first;
			aList->first = aList->last->next;
			aList->last->next = aList->first->prev = NULL;
		}
	}
}
