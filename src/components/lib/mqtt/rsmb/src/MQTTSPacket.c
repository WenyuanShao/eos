...
extern char *mqtt_dummy_recvfrom(int sock, char *msg, int buffer_len, int flag, int *len);
...
/* #if !defined(NO_BRIDGE)
	...
	*clientAddr = mqtt_dummy_recvfrom(sock, msg, max_packet_size, 0, &len);
	n = len;
	//int i = 0;
	//for (i = 0; i < len; i++) {
	//	printc("%x_",msg[i]);
...
extern int mqtt_dummy_sendto(int sock, char *pkt_buf, int len, int flag, int port, unsigned long *dst_addr);
...
int MQTTSPacket_sendPacketBuffer(int socket, char* addr, PacketBuffer buf)
{
	...
	*(port - 1) = '\0';
	...
	else
	{
		...
		rc = mqtt_dummy_sendto(socket, buf.data, buf.len, 0, cliaddr.sin_port, &cliaddr.sin_addr.s_addr);
