...
extern int mqtt_dummy_select(void);
/**
...
int Socket_getReadySocket(int more_work, struct timeval *tp)
{
	...
	if (s.cur_clientsds == NULL)
	{
		...
		rc = mqtt_dummy_select();
		if (rc == SOCKET_ERROR)
		{
			printc("dummy_select fail\n");
	...
	{	
		...
		while (ListNextElement(s.listeners, &current))
		{
			 ...
			 */
			if (list->protocol == 0 && rc == 1) { /* if this is a new connection attempt */
				printc("error!!!\n");
				newConnection(list);
			}
		}
